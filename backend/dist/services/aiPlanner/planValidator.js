...
exports.validateAiGeneratedPlan = validateAiGeneratedPlan;
 ...
 */
function convertPlanToValidateInput(plan) {
 ...
 */
function detectHallucinatedUnits(plan) {
    const hallucinated = [];
 ...
 */
async function validateAiGeneratedPlan(plan, courseCode, completedUnits, specialisation) {
    const plannedTerms = convertPlanToValidateInput(plan);
    const selectedSpecialisations = specialisation ? [specialisation] : [];
    let validationResult;
    try {
        validationResult = await (0, planValidationService_1.validatePlan)({
            courseCode,
            completedUnits,
