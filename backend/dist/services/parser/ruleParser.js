...
function simplifyRuleNode(node) {
    if (node.type === "UNIT" ||
        node.type === "POINTS" ||
        node.type === "WAM") {
        return node;
...
function splitTopLevelByWord(text, operator) {
    const result = [];
    let depth = 0;
    let start = 0;
...
function extractPrefixes(text) {
    const matches = Array.from(text.matchAll(/\b([A-Z]{4})(?=\/|\s+units|\b)/g)).map((m) => m[1]);
...
function extractPointsConstraintMetadata(text) {
    ...
    if (!prefixes && !levels && !category && !scope)
        return undefined;
    return {
        ...(prefixesOverride
...
function branchMentionsCourse(text, courseCode) {
    if (!courseCode)
        return false;
    return new RegExp(`\\b${courseCode}\\b`).test(text);
}
function extractRequirementAttachedToCurrentCourse(text, courseCode) {
    if (!courseCode)
        return null;
...
function isSatisfiedEnrollmentBranch(text, courseCode) {
    const clean = normalizeText(text);
    if (!courseCode)
        return false;
    if (!/enrolment in/i.test(clean))
        return false;
    if (!branchMentionsCourse(clean, courseCode))
        return false;
    const unitCodes = clean.match(/[A-Z]{4}\d{4}/g) ?? [];
    const points = clean.match(/\d+\s+points/gi) ?? [];
    return unitCodes.length === 0 && points.length === 0;
}
function preprocessForCourse(text, courseCode) {
    if (!courseCode)
...
function parseAtom(text, options) {
    const clean = stripOuterParens(normalizeText(text));
    if (!clean || isNil(clean))
        return null;
    ...
    let pointsMatches = Array.from(clean.matchAll(/(\d+)\s+points/gi)).map((m) => Number(m[1]));
    const unitCountMatches = [
        ...Array.from(clean.matchAll(/(\d+)\s+units/gi)).map((m) => Number(m[1])),
        ...Array.from(clean.matchAll(/\b(one|two|three|four|five|six|seven|eight|nine|ten)\s+units/gi)).map((m) => wordToNumber[m[1].toLowerCase()]),
    ];
...
function parseExpression(text, options) {
    ...
    // then keep only X.
    const attachedRequirement = extractRequirementAttachedToCurrentCourse(clean, options?.courseCode);
...
function parseRule(text, options) {
    if (!text || isNil(text))
        return null;
    const clean = normalizeText(text);
