...
function validateRequestShape(payload) {
    ...
    payload.plan.forEach((term, index) => {
...
function flattenPlannedUnits(plan) {
...
function validateSemesterUnitLoad(plan) {
    const issues = [];
...
async function loadAiDataPrerequisites(programCode) {
 ...
 */
function isRawPrerequisiteSatisfied(raw, completedUnits) {
    const groups = parseRawPrerequisiteUnits(raw);
    if (!groups || groups.length === 0)
        return true; // no prereqs
    if (groups.some((g) => g.length === 0))
        return true; // empty group = no prereqs
    // OR semantics: any group fully satisfied → pass
    return groups.some((group) => group.every((code) => completedUnits.has(code)));
}
...
function getUnitLevel(unitCode) {
    const match = unitCode.match(/\d/);
    return match ? Number(match[0]) : null;
}
function unitMatchesPointConstraints(unitCode, constraints) {
    if (!constraints)
        return true;
    if (constraints.prefixes?.length) {
        const matchesPrefix = constraints.prefixes.some((prefix) => unitCode.startsWith(prefix));
        if (!matchesPrefix)
            return false;
    }
...
function countCompletedPoints(completedUnits, constraints) {
    let points = 0;
...
function isRuleSatisfied(rule, completedUnits) {
    if (!rule)
        return true;
    if (rule.type === "UNIT") {
    ...
    if (rule.type === "POINTS") {
        const completedPoints = countCompletedPoints(completedUnits, rule.constraints);
...
function validatePrerequisites(params) {
    ...
    for (const term of sortedPlan) {
        for (const unitCode of term.units) {
            ...
            const rawPrereq = unit._prerequisiteRaw ?? null;
            if (rawPrereq !== undefined) {
                ...
                const prereqRule = unit.prerequisites_parsed;
...
function validateCorequisites(params) {
    ...
    for (const term of sortedPlan) {
        ...
        for (const unitCode of term.units) {
            ...
            if (!unit)
                continue;
            const coreqRule = unit.corequisites_parsed;
...
function validateIncompatibilities(params) {
    ...
    for (const term of params.plan) {
        for (const unitCode of term.units) {
            ...
            if (!unit)
                continue;
            const incompatRule = unit.incompatibilities_parsed;
            if (!incompatRule)
                continue;
            if (isRuleSatisfied(incompatRule, allTakenOrPlanned)) {
                const messageTarget = ruleToText(incompatRule);
                const severity = completedSet.has(messageTarget) || plannedSet.has(messageTarget)
                    ? "fail"
                    : "warning";
                issues.push({
...
function countPointsForUnits(unitCodes) {
    // Assumption: each unit is worth 6 points
    return unitCodes.length * 6;
}
function filterUnitsByLevel(unitCodes, levels) {
...
function evaluateGroupRule(params) {
    ...
    if (rule.type === "POINTS_FROM_GROUP") {
        ...
        const selectedFromGroup = relevantGroupUnits.filter((unitCode) => selectedUnits.has(unitCode));
        const constrainedUnits = filterUnitsByLevel(selectedFromGroup, rule.constraints?.levels);
    ...
    if (rule.type === "POINTS_TOTAL_FROM_GROUP_SET") {
        const unitCodesFromGroups = rule.groups.flatMap((code) => groupUnitsByCode.get(code) ?? []);
        const selectedFromGroups = unitCodesFromGroups.filter((unitCode) => selectedUnits.has(unitCode));
...
function normaliseSpecialisation(value) {
    return value.trim().toLowerCase();
}
function getSelectedSpecialisationGroupCodes(selectedSpecialisations, courseCode) {
    ...
    for (const specialisation of selectedSpecialisations) {
        ...
        if (courseCode === "41680") {
            ...
            if (value === "business information and logistics management" ||
                value === "sp-bsimg" ||
                value === "sp_bsimg") {
            ...
            if (value === "international business" ||
...
function shouldValidateGroup(params) {
    ...
    if (courseCode === "41680") {
        ...
        if (groupCode.startsWith("GROUP_") ||
...
async function validateGroupRules(params) {
    ...
    const selectedSpecialisationGroupCodes = getSelectedSpecialisationGroupCodes(params.selectedSpecialisations, params.courseCode);
    const groups = await (0, courseService_1.fetchGroupsForCourse)(params.courseCode);
    const groupsWithUnits = await Promise.all(groups.map(async (group) => ({
        ...group,
        units: await (0, courseService_1.fetchUnitsForGroup)(group.id),
    })));
    const groupUnitsByCode = new Map();
    for (const group of groupsWithUnits) {
        groupUnitsByCode.set(group.group_code, group.units.map((unit) => unit.code));
    }
    for (const group of groupsWithUnits) {
...
function validate62510Rules(params) {
    ...
    if (nonAppliedSpecialisations.length > 2) {
        issues.push({
            ...
            message: "Students may complete a maximum of two specialisations, excluding Applied Computing.",
        });
    }
    // Rule: Applied Computing students may complete only one specialisation.
    const hasAppliedComputing = params.selectedSpecialisations.some((specialisation) => specialisation.trim().toLowerCase() === "applied computing" ||
        specialisation.trim().toLowerCase() === "sp_apcmp");
...
function validateCoursePoints(params) {
    ...
    if (params.course.min_points !== null &&
    ...
    if (params.course.max_points !== null &&
...
async function validatePlan(payload) {
    const issues = [];
    ...
    const course = await (0, courseService_1.fetchCourseByCode)(payload.courseCode);
    if (!course) {
        issues.push({
            category: "course",
            severity: "fail",
    ...
    issues.push(...validateUnitMembership({
        completedUnits: payload.completedUnits,
        plannedUnits,
        courseUnitCodes,
    }));
    issues.push(...validateDuplicateUnits({
        completedUnits: payload.completedUnits,
        plannedUnits,
    }));
    issues.push(...validateSemesterUnitLoad(payload.plan));
    issues.push(...validateAvailability({
        plan: payload.plan,
        unitByCode,
    }));
    ...
    issues.push(...validateCorequisites({
        plan: payload.plan,
        completedUnits: payload.completedUnits,
        unitByCode,
    }));
    issues.push(...validateIncompatibilities({
        plan: payload.plan,
        completedUnits: payload.completedUnits,
        unitByCode,
    }));
    issues.push(...(await validateGroupRules({
        courseCode: payload.courseCode,
        selectedSpecialisations: payload.selectedSpecialisations,
        completedUnits: payload.completedUnits,
        plannedUnits,
    })));
    issues.push(...validateCourseSpecificRules({
        courseCode: payload.courseCode,
        selectedSpecialisations: payload.selectedSpecialisations,
        completedUnits: payload.completedUnits,
        plannedUnits,
