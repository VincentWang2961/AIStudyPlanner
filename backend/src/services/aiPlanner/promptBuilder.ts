...
export function buildSystemPrompt(): string {
  return [
    ...
    '⛔ **RESEARCH PAIR RULE (ABSOLUTE):** CITS5014 and CITS5015 are a BOUND PAIR. Either include BOTH (in consecutive semesters within the standard 4-semester plan, 5014 before 5015) or include NEITHER. Before placing CITS5014, verify you have room for BOTH within the 4 standard semesters. If not, do NOT place either and do NOT create extra semesters — fill those slots with normal electives instead.',
    '⛔ **NO EXTRA SEMESTERS:** Do NOT create a 5th or additional semester to squeeze in Layer 3 requests. If a request cannot fit within the standard semester count, DROP IT COMPLETELY and warn the student to consider extending their study duration.',
    '',
    '## Reasoning Process',
    '',
    ...
    '',
    '**EXAMPLE: AI + Research conflict (most common case)**',
    'S1 2027 S1-only candidates: CITS4401(🔒), CITS5505(🔒), CITS5508(🎯), CITS4404(🎯), CITS5014(💬) = 5 units, only 4 slots → DROP CITS5014 (lowest priority). Since CITS5014 is dropped, CITS5015 is also dropped (bound pair). Fill S2 2027 slots with electives instead.',
    'S2 2027: CITS5206(🔒) + CITS5017(🎯, needs CITS5508) + 2 electives = 4 slots.',
    '',
