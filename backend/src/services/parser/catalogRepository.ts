...
export async function upsertCatalog(client: PoolClient, catalog: Catalog): Promise<void> {
  for (const course of catalog.courses) {
    ...
    for (const unit of course.units) {
      if (!unit.code || !unit.title) continue;
    ...
    for (const group of course.pdf_rules.groups ?? []) {
      ...
      const groupId = insertGroupResult.rows[0].id as number;
