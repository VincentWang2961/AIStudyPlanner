...
interface GroupDef {
  group_code: string; name: string; rule_text: string;
  rule_json: unknown; unit_codes: string[];
}
 ...
 * </tr>
 */
function extractRowUnitCodes(html: string): string[] {
  const codes: string[] = [];
  // Split into rows
...
interface Section { name: string; start: number; end: number; isSpec: boolean; specCode?: string }
...
function findSections(html: string): Section[] {
  ...
  for (const spec of specs) {
    ...
    while ((gm = groupRegex.exec(html)) !== null) {
      if (gm.index < kwIdx || gm.index > sectionEnd) continue;
      const tableStart = html.indexOf("<table", gm.index);
      if (tableStart < 0 || tableStart > sectionEnd) continue;
      const tableEnd = html.indexOf("</table>", tableStart) + 8;
...
async function scrapeCourse(code: string) {
  ...
  const title = stripTags(
    html.match(/<h2[^>]*id="pagetitle"[^>]*>(.*?)<\/h2>/)?.[1] ||
...
async function upsertCourse(course: any) {
  await prisma.courses.upsert({
    ...
    update: {
      ...
      max_points: course.max_points, specialisations: course.specialisations as any,
    },
    create: {
  ...
  await prisma.$executeRawUnsafe(
    `DELETE FROM group_units WHERE group_id IN (SELECT id FROM course_groups WHERE course_code = $1)`, course.code);
  await prisma.$executeRawUnsafe(
    `DELETE FROM course_groups WHERE course_code = $1`, course.code);

  for (const group of course.groups) {
    await prisma.course_groups.create({
      data: {
        course_code: course.code, group_code: group.group_code,
        name: group.name, rule_text: group.rule_text,
        rule_json: group.rule_json as any,
      },
    });
      ...
      try {
        await prisma.$executeRawUnsafe(
          `INSERT INTO group_units (group_id, unit_code) 
           ...
           WHERE course_code = $2 AND group_code = $3 ON CONFLICT DO NOTHING`,
          uc, course.code, group.group_code);
      } catch {}
    }
  }
